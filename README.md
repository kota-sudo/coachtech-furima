...
> **DB設計の整合について**：テーブル仕様書どおりの **9 テーブル構成**で、`purchases.payment_method`（integer：1=コンビニ支払い / 2=カード支払い）に支払い方法を保持します。最新の正式定義は [docs/table_design.md](docs/table_design.md) を参照してください。
...
- 支払い方法は `purchases.payment_method`（1=コンビニ支払い / 2=カード支払い）として保持
- `STRIPE_SECRET` が設定されている場合は、コンビニ・カードいずれも **Stripe Checkout**（カード=card / コンビニ=konbini）に遷移し、決済成功後に購入が確定します
- Stripe キー未設定時は決済画面を経由せず購入を確定します
...
- 正式なテーブル定義は `docs/table_design.md`（実装準拠・9テーブル）に集約
- 支払い方法は `purchases.payment_method`（integer）で保持（マスタテーブルは作成しない）
